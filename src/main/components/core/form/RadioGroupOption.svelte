...
<script lang="ts">
    let {         id,
        ...
        let styleClass = $derived(themedTWMerge("shrink-0 h-[.65lh] w-[.65lh] rounded-full border data_checked:border-4",
    ...
    function mutationCallback(mutationsList)
    {
        for (const mutation of mutationsList) 
        {
            ...
            {
                if (mutation.target.getAttribute("data-state") === "unchecked")
                {
    ...
    let style=$derived((fetchedChecked && dynamicColorTheme) ? `border:4px solid ${getDynamicColorTheme(dynamicColorTheme, colorVariant).coreColor} !important;` : "");
</script>
