...
<script lang="ts">
    ...
    const styleClass = $derived(themedTWMerge("px-4 py-3.5 border-b-2 w-full flex justify-start items-center",
        `border-${colorVariant}-text`,
        getBaseColorClassesForColorCategoryStyleVariant(colorVariant)
    ));

    const style=$derived((dynamicColorTheme) ? `border-color:${getDynamicColorTheme(dynamicColorTheme, colorVariant).textColor} !important; ${getBaseColorStyleForDynamicColorTheme(dynamicColorTheme, colorVariant)}` : "");
</script>
